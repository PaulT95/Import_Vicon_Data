...
if (isempty(Ref_Dev) == false && nargout > 2)
    ...
    for k=1:length(Labels.Analog)
        ...
        if(~isvarname(Labels.Analog{k})) %check whether the field name is not a valid for the struch
            %check if it is possible to keep original name only because
            %people use space or - in the name which is not allowed
            if isvarname(regexprep(Labels.Analog{k},{' '; '-'},'_'))
                Labels.Analog{k} = regexprep(Labels.Analog{k},' ','_');
            else
...
if (isempty(Ref_MOut) == false && nargout > 4) %just check, if there are not ModelOutputs data, skip
    ...
    %in case no marker was loaded and thus no reference and subject was
    %created ahead
    if isempty(Subject) && isempty(Ref_Mrk)
        position = find(~cellfun(@isempty,Labels.ModelOutputs),1);%find first not empty as it depends on model used
        tmp = split(Labels.ModelOutputs{position} ,':');
        Subject = tmp{1};
        Ref_Mrk = size(Data,1);
    end
